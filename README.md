...
Current implementation status: **Phase 0.2.1 marker-based command blocks** in `apps/desktop`.

## Current scope (Phase 0.2.1)
...
- Block lifecycle tracking:
  ...
  - Block ends from shell integration markers (`OSC 133` command done + exit code)
  - Prompt-suffix heuristic is retained only as a fallback when markers are unavailable
...
3. Confirm each block status changes from `running` to `success` and shows `exit 0`.
4. Run `false`; confirm block ends as failed with a non-zero exit code.
5. Run `sleep 30`, press `Ctrl+C`, and confirm command interruption and failed block status.
6. Resizing the window keeps terminal usable.
7. Copy selected output and paste back into terminal.
